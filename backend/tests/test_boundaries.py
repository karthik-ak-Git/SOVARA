...
def test_models_registry_lists_configured_fallback(client) -> None:
    # Default client: Ollama is down -> its configured record stays visible,
    # honestly marked unavailable. (A running LM Studio adds its own live
    # rows; this test only pins the down-runtime contract.) Routing is auto.
    r = client.get("/api/v1/models")
    assert r.status_code == 200
    body = r.json()
    by_id = {i["id"]: i for i in body["items"]}
    assert "llama3.1" in by_id
    item = by_id["llama3.1"]
