...
/**
 * Hardware-aware context sizing: the largest discrete tier that fits free VRAM
 * (with GPU) or free RAM (CPU-only), at the given KV cost, with the configured
 * safety margin. Never hard-codes a GPU size.
 */
...
export interface RouterContext {
  ...
  /** Optional score delta for a future tie breaker. */
  tieThreshold?: number
}
...
export async function routeModel(ctx: RouterContext): Promise<ModelRoutingDecision> {
  ...
  if (scored.length >= 2 && ctx.tieBreaker) {
    ...
    const threshold = ctx.tieThreshold ?? 12
    if (delta <= threshold) {
      try {
        const result = await ctx.tieBreaker(task.kind, scored.slice(0, 4).map((s) => ({
          id: s.model.modelId,
          label: `${s.model.displayName ?? s.model.modelId} (${s.capabilities.join('/') || 'general'}, ctx=${s.contextLength})`,
        })))
        const winner = scored.find((s) => s.model.modelId === result.decision.id)
        if (winner && winner !== top) {
          scored = [winner, ...scored.filter((s) => s !== winner)]
          winner.reason += `, tie-break (Δ=${delta.toFixed(1)})`
        }
      } catch {
        // Keep the deterministic winner when an optional policy fails.
      }
    }
  }
