  ...
  it('only HttpClient performs network operations (fetch or POST)', () => {
    ...
    for (const f of scan('src')) {
      ...
      if (/\bfetch\s*\(/.test(txt)) {
        ...
        expect(rel, `fetch outside HttpClient: ${f}`).toMatch(/(main\/network\/HttpClient\.ts|main\/services\/hfCatalog\.ts|main\/services\/explorerCatalog\.ts|main\/services\/modelDownloads\.ts|main\/services\/skillsScanner\.ts)$/)
      }
    }
  })
