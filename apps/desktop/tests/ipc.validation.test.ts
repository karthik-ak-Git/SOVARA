...
describe('IPC contracts — Zod strict validation', () => {
  ...
  it('IPC channel whitelist is the single source', () => {
    ...
    expect(invoke).toContain('updates:checkNow')
    expect(invoke).toContain('updates:download')
    expect(invoke).toContain('updates:install')
    const on = Object.entries(IPC_CHANNELS)
      .filter(([, v]) => v.type === 'on')
      .map(([k]) => k)
    expect(on).toContain('events:download')
    expect(on).toContain('updates:event')
