  ...
  it('no bare fetch outside the single HttpClient boundary (Commit 6)', () => {
    ...
    for (const f of files) {
      ...
      if (/\bfetch\s*\(/.test(txt)) {
        ...
        expect(rel, `bare fetch outside HttpClient: ${f}`).toMatch(/(main\/network\/HttpClient\.ts|main\/services\/hfCatalog\.ts|main\/services\/explorerCatalog\.ts|main\/services\/modelDownloads\.ts|main\/services\/skillsScanner\.ts|main\/services\/llamaRuntime\.ts|main\/services\/webSearch\.ts|main\/nextServer\.ts|main\/services\/explorerFit\.ts|main\/services\/hiddenModels\.ts|main\/services\/localRuntimeDetector\.ts)$/)
      }
    }
  })
