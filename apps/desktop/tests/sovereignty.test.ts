  ...
  it('no bare fetch outside the single HttpClient boundary (Commit 6)', () => {
    ...
    for (const f of files) {
      ...
      if (/\bfetch\s*\(/.test(txt)) {
        // Exceptions: HttpClient (loopback inference), hfCatalog + explorerCatalog (Hub API), modelDownloads (Hub file downloads), skillsScanner (skill import from URL), llamaRuntime (one-time pinned llama.cpp binary provisioning — inference itself stays loopback), nextServer (loopback health-check of the internal Next.js server), explorerFit (model metadata probing), hiddenModels (hidden model probing)
        const rel = f.replace(/\\/g, '/')
        expect(rel, `bare fetch outside HttpClient: ${f}`).toMatch(/(main\/network\/HttpClient\.ts|main\/services\/hfCatalog\.ts|main\/services\/explorerCatalog\.ts|main\/services\/modelDownloads\.ts|main\/services\/skillsScanner\.ts|main\/services\/llamaRuntime\.ts|main\/nextServer\.ts|main\/services\/explorerFit\.ts|main\/services\/hiddenModels\.ts|main\/services\/localRuntimeDetector\.ts)$/)
      }
    }
  })
